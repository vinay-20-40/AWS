      ...
      *    PROGRAMMER: AWS                                            *
      *    DATE:       07/01/2025                                      *
      *    REMARKS:    BATCH REPORTING FOR DAILY TRANSACTIONS         *
      *                                                                *
      ******************************************************************
      ...
      *****************************************************************
       ...
       COPY CVTRA07Y.
       01  TRANREPT-STATUS.
           05  TRANREPT-STAT1      PIC X.
           05  TRANREPT-STAT2      PIC X.
       01 DATEPARM-STATUS.
      ...
      * Open the cross reference file.
       0200-CARDXREF-OPEN.
           ...
           END-IF
           EXIT.
      * Open the transaction type file.
       0300-TRANTYPE-OPEN.
           MOVE 8 TO APPL-RESULT.
           ...
           END-IF
           EXIT.
      * Open the transaction category file.
       0400-TRANCATG-OPEN.
           MOVE 8 TO APPL-RESULT.
      ...
      * Open the date parameter file.
       0500-DATEPARM-OPEN.
           ...
           IF APPL-AOK
              CONTINUE
           ELSE
              DISPLAY 'ERROR OPENING DATE PARAMETER FILE'
              MOVE DATEPARM-STATUS TO IO-STATUS
              PERFORM 9910-DISPLAY-IO-STATUS
              PERFORM 9999-ABEND-PROGRAM
           END-IF
           EXIT.
      * Write a transaction to the report.
       ...
       1111-WRITE-REPORT-REC.
           ...
           END-IF
           EXIT.
      * Lookup routines with complete FILE STATUS checking.
       1500-A-LOOKUP-XREF.
           READ XREF-FILE INTO XREF-RECORD
              INVALID KEY
                 ...
                 PERFORM 9999-ABEND-PROGRAM
           END-READ
           IF CARDXREF-STATUS NOT = '00' AND CARDXREF-STATUS NOT = '23'
              MOVE CARDXREF-STATUS TO IO-STATUS
              PERFORM 9910-DISPLAY-IO-STATUS
              PERFORM 9999-ABEND-PROGRAM
           END-IF
       ...
       1500-B-LOOKUP-TRANTYPE.
           READ TRANTYPE-FILE INTO TRAN-TYPE-RECORD
              INVALID KEY
                 ...
                 PERFORM 9999-ABEND-PROGRAM
           END-READ
           IF TRANTYPE-STATUS NOT = '00' AND TRANTYPE-STATUS NOT = '23'
              MOVE TRANTYPE-STATUS TO IO-STATUS
              PERFORM 9910-DISPLAY-IO-STATUS
              PERFORM 9999-ABEND-PROGRAM
           END-IF
       ...
       1500-C-LOOKUP-TRANCATG.
           READ TRANCATG-FILE INTO TRAN-CAT-RECORD
              INVALID KEY
                 ...
                 PERFORM 9999-ABEND-PROGRAM
           END-READ
           IF TRANCATG-STATUS NOT = '00' AND TRANCATG-STATUS NOT = '23'
              MOVE TRANCATG-STATUS TO IO-STATUS
              PERFORM 9910-DISPLAY-IO-STATUS
              PERFORM 9999-ABEND-PROGRAM
           END-IF
           EXIT.
      * Close routines with consistent FILE STATUS checking.
       9000-TRANFILE-CLOSE.
           MOVE 8 TO APPL-RESULT.
           CLOSE TRANSACT-FILE
           IF TRANFILE-STATUS = '00'
       ...
       9500-DATEPARM-CLOSE.
           ...
           IF APPL-AOK
              CONTINUE
           ELSE
              DISPLAY 'ERROR CLOSING DATE PARAMETER FILE'
              MOVE DATEPARM-STATUS TO IO-STATUS
              PERFORM 9910-DISPLAY-IO-STATUS
              PERFORM 9999-ABEND-PROGRAM
           END-IF
